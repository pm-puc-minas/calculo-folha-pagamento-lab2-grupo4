...
public class FolhaPagamentoService {
    ...
    public FolhaPagamento getFolha(Funcionario funcionario, YearMonth periodo) throws Exception {
        ...
        Optional<FolhaPagamento> folhaExistente =
                folhaPagamentoRepository.findByFuncionarioAndPeriodo(funcionario, periodo);
    ...
    public FolhaPagamento gerarFolha(Funcionario funcionario, YearMonth periodo) throws Exception {
        ...
        BigDecimal salarioLiquido = calcularSalarioLiquido(
                funcionario,
                totalDescontos,
                totalProventos
        );
    ...
    public HashMap<String, BigDecimal> calcularDescontos(Funcionario funcionario, YearMonth periodo) throws Exception {
        ...
        for (IDesconto desconto : descontoFactory.criarDescontos()) {

            BigDecimal valor = desconto.calcular(funcionario, periodo);

            descontosCalculados.put(desconto.getNome(), valor);
            total = total.add(valor);
        }

        descontosCalculados.put("total", total);
    ...
    public HashMap<String, BigDecimal> calcularProventos(Funcionario funcionario, YearMonth periodo) throws Exception {
        ...
        for (IProvento provento : proventos) {

            BigDecimal valorProvento = provento.calcular(funcionario, periodo);
            proventosCalculados.put(provento.getNome(), valorProvento);

            // Não somar ValeAlimentacao e ValeTransporte ao total (regra do seu projeto)
            if (!provento.getNome().equals("ValeAlimentacao") &&
                !provento.getNome().equals("ProventoValeTransporte")) {
    ...
    public HashMap<String, BigDecimal> calcularEncargoSocial(Funcionario funcionario) throws Exception {
        ...
        for (IEncargoSocial encargo : encargos) {

            BigDecimal valorEncargo = encargo.calcular(funcionario);
            encargoSocialCalculados.put(encargo.getNome(), valorEncargo);
        }
    ...
    public void deleteByFuncionarioAndPeriodo(Funcionario funcionario, YearMonth periodo) {
        ...
        FolhaPagamento folha = folhaPagamentoRepository
                .findByFuncionarioAndPeriodo(funcionario, periodo)
